...
export default function BottomNav() {
  ...
  return <nav className="glass safe-bottom fixed bottom-0 left-0 right-0 z-50 border-x-0 border-b-0 md:hidden"><div className="flex h-16 items-center justify-around px-2">{items.map((item) => { const Icon = item.icon; const active = item.href === "/" ? pathname === "/" : pathname.startsWith(item.href); return <Link key={item.id} href={item.href} className={cn("flex min-w-14 flex-col items-center gap-1 text-[10px] font-medium", active ? "text-primary" : "text-muted-foreground")}><span className={cn("flex size-9 items-center justify-center rounded-xl", item.create ? "gradient-bg -mt-7 size-14 rounded-2xl text-primary-foreground shadow-lg glow-sm ring-4 ring-background" : active && "bg-primary/15")}><Icon className={cn(item.create ? "size-6" : "size-5")} /></span>{!item.create && <span>{item.label}</span>}</Link>; })}</div></nav>;
}
